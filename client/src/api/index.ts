...
export async function joinGame(uuid: string, user: string): Promise<GameSession> {
    const response = await fetch(`${API_URL}/game/${uuid}/join`, {method: 'POST', headers: {
...
export async function createGame(user: string): Promise<GameSession> {
    console.log(user);
    const response = await fetch(`${API_URL}/game`, {method: 'POST', headers: {
        "Content-Type": "application/json"
